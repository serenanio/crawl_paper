...
data = collection.find({'cite_num':{'$gte':10000}})
...
journal = list(paper_data['journal'])
year = list(paper_data['year'])
volume = list(paper_data['volume'])
# print(author)
print(len(paper))
class CitenumSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(len(paper)):
            ...
            print(url)
            yield scrapy.Request(url=url, headers=header, callback=self.parse,meta={"type":type[i],"paper_name": paper[i],"journal":journal[i],
    ...
    def parse(self, response):
        ...
        collection.update_many({'journal': response.meta['journal'], 'year':response.meta['year'], 'paper_name': response.meta['paper_name']},
                               {'$set': {'cite_num': cite_num, 'creat_time': creat_time, 'cite_from': 'bing'}})

        # paper_item['paper_name'] = response.meta['paper_name']
        # paper_item['type'] = response.meta['type']
        # paper_item['journal'] = response.meta['journal']
        # paper_item['author_link'] = response.meta['author_link']
        # paper_item['author_list'] = response.meta['author_list']
        # paper_item['volume'] = response.meta['volume']
